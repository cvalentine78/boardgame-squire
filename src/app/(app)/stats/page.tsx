...
import { getAllSessionPlayers, getAllScores } from '@/lib/db'
...
export default function StatsPage() {
  ...
  useEffect(() => {
    Promise.all([getAllSessionPlayers(), getAllScores()]).then(([rows, scoreRows]) => {
      const completed = (rows ?? []).filter((r: Row) => r.game_sessions?.status === 'completed')

      // Build per-session totals from scores: sessionTotals[sessionId][playerName] = total points
      ...
      // Per-game stats with per-player breakdown
